...
仓库当前代码还包含尚未发版的搜索页标签稳定性修复、搜索页卡片快捷勾选框支持，以及“已看完阈值”可配置项，详见 [CHANGELOG.md](./CHANGELOG.md) 中的 `未发布` 部分。
...
- 进一步收紧缩略图标签宿主选择，尽量减少对 B 站原生预览和卡片布局的干扰
- 为搜索结果页视频卡片补充底部信息行最右侧的快捷勾选框
- 在控制面板中新增“已看完阈值”可配置项，支持滑块与输入框联动调整
...
5. 在视频详情页中，可以直接查看当前视频状态，并手动修正记录
6. 在缩略图卡片左上角，会显示较小的已看状态徽标
7. 在 UP 主空间主页和搜索结果页的视频卡片底部信息行右侧，可以点击小方块快捷标记 `已看完`
...
已发布版本 `v1.3.4` 当前采用以下结构：

- `background.js`：负责设置项管理、已看记录存储、自动完成阈值判定和跨标签页消息广播
- `content.js`：负责页面功能注入、自动记录观看进度、视频页状态展示、缩略图徽标、空间页 / 搜索页快捷标记和合集增强
